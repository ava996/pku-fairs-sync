...
3. 左侧 **权限管理** → 搜索并开通 `bitable:app`（含读+写），以及视图权限 `base:view:read`、`base:view:write_only`（自动按周建视图需要，缺失则脚本只同步数据、不建视图）
...
| 地点 | 文本 | |
| 详情链接 | 文本 | Markdown 链接 |
| 时间范围 | 单选 | 自然周标签（如 `9/14-9/20`，周一起始） |
| 推荐等级 | 单选 | 🔥强烈推荐 / 推荐 / 一般 |
| 推荐理由 | 文本 | 命中依据 |
| 备注 | 文本 | 代理发布说明等 |
| 参加状态 | 单选 | **手动标记**：⭐想参加 / 📅已报名 / ✅已参加 / ❌不去了。脚本更新不覆盖 |
| 是否参加 | 单选 | **手动标记**：✅是 / ❌否。脚本更新不覆盖 |
| 宣讲会ID | 文本 | 去重键 |

## 更新机制

- **全量爬取 + 按需写入**：每次运行拉取接口全部数据，按"宣讲会ID"与表内比对，新增缺失记录、部分更新已有记录
- **部分更新**：脚本只提交自己管理的字段（时间/地点/推荐等），**用户手动维护的字段（参加状态、是否参加）永不被覆盖**
- **自然周标签**：每次运行按宣讲会举办日期重算其所属自然周（周一起始），写入 `时间范围` 字段

## 视图

- **全部宣讲会**（默认，按开始时间升序）
- **📅 9/14-9/20、📅 9/21-9/27 …**（每个自然周一张，filter `时间范围=该周`，按开始时间升序，自动创建）

> 每周脚本会检查数据中出现的新自然周，自动补建对应视图；「是否参加」等表级字段会自动带入每张周视图。
